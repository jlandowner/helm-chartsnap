...
func (o *ChartSnapshotter) prependSnapshotHeader(data []byte) []byte {
	header := (&v1alpha1.Header{SnapshotVersion: o.Config.SnapshotVersion}).ToString()
...
func (o *ChartSnapshotter) SnapshotFile() string {
	...
	if o.Config.SnapshotFileExt != "" {
		// append snapshot file extension
		o.snapshotFile += "." + o.Config.SnapshotFileExt
	}
...
func (o *ChartSnapshotter) Snap(ctx context.Context) (result *SnapshotResult, err error) {
	...
	sv.TestSpec.Merge(o.Config)
	o.Config = sv.TestSpec
	log().Debug("compluted config", "config", o.Config)
	...
	if o.Config.SnapshotVersion == "" {
		if snap.IsMultiSnapshots(o.SnapshotFile()) {
			...
			o.Config.SnapshotVersion = SnapshotVersionV1
			...
			o.Config.SnapshotVersion = SnapshotVersionV2
		} else {
			// later: use snapshot version from snapshot file if exists
			o.Config.SnapshotVersion = snapVersion
		}
	}
	...
	// take snapshot
	log().Debug("taking snapshot", "version", o.Config.SnapshotVersion)

	switch o.Config.SnapshotVersion {
	...
	default:
		log().Error("unsupported snapshot version. use latest", "version", o.Config.SnapshotVersion, "latest", SnapshotVersionLatest)
		o.Config.SnapshotVersion = SnapshotVersionLatest
		return o.snapV3(sv.TestSpec, out)
	}
}
